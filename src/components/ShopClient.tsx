...
function ProductCard({
  ...
  return (
    <motion.article
      ...
      className="group"
    >
      <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.3 }} className="flex flex-col h-full">
        <div className="bg-surface rounded-brand aspect-[4/5] flex items-center justify-center mb-5 border border-border group-hover:border-accent-soft transition-colors overflow-hidden">
          <motion.div whileHover={{ scale: 1.05 }} transition={{ duration: 0.5 }} className="flex items-center justify-center">
            <ProductSvg />
          </motion.div>
        </div>
        <div className="flex-1 flex flex-col px-1">
          ...
          <motion.button
            ...
            className={`w-full py-3 rounded-full text-sm font-semibold transition-colors ${
              added ? 'bg-green-700 text-white' : 'bg-accent text-white hover:bg-accent-dark'
            }`}
          >
              ...
              {cartProducts.map(({ product_id, quantity, product }) => (
                <li key={product_id} className="flex items-start gap-4 pb-6 border-b border-border last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-fg-muted text-xs mb-3">{product ? formatPrice(product.price_cents, product.currency) : ''}</p>
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => onRemove(product_id)} className="text-xs text-fg-muted hover:text-fg transition-colors">
                        Remove
                      </button>
                    </div>
                  </div>
        ...
        {cart.length > 0 && (
          <div className="px-6 py-5 border-t border-border">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="font-serif text-fg text-xl">{formatPrice(total, 'EUR')}</span>
            </div>
      ...
      <div className="min-h-screen bg-bg pt-16">
        <div className="bg-bg border-b border-border">
          <div className="max-w-6xl mx-auto px-6 py-16 flex items-end justify-between">
            <div>
              <p className="text-xs font-medium tracking-widest uppercase text-accent mb-3">All Arrangements</p>
              <h1 className="font-serif text-fg leading-none" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)' }}>
                The Collection
              </h1>
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          {products.length === 0 ? (
            <div className="py-32 text-center">
              <div className="flex justify-center mb-8"><ProductSvg /></div>
              <h2 className="font-serif text-fg text-2xl mb-3">Collection coming soon</h2>
              <p className="text-fg-muted mb-8">Our next arrangements are being carefully composed. Check back soon.</p>
              <Link href="/" className="inline-flex items-center gap-2 text-accent font-medium text-sm hover:text-accent-dark transition-colors">
                ← Back to home
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product, i) => (
                <ProductCard key={product.product_id} product={product} index={i} onAddToCart={addToCart} />
              ))}
            </div>
          )}
        </div>
        ...
        {cartCount > 0 && !cartOpen && (
          <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30">
            ...
            >
              <span className="flex items-center justify-center w-6 h-6 bg-accent rounded-full text-xs font-bold">{cartCount}</span>
              View Cart
